...
MACHINE=ast2600-qcom-dc-scm-v1
OBJ=qcom-dc-scm-obj
...
make O="$OBJ" -s clean
make O="$OBJ" -s "$CONFIG"
echo CONFIG_TARGET_QUALCOMM_DC_SCM_V1=y >> "$OBJ/.config"
{ set +x; } 2>/dev/null
...
set -x
make -j8 O=$OBJ -s oldconfig 2> /dev/null
{ set +x; } 2>/dev/null
...
dd if=/dev/zero of=$TEST_IMG count=1 bs=64M
...
echo
echo "$QEMU_SYSTEM_ARM" -M qcom-dc-scm-v1-bmc -nographic -drive file=$TEST_IMG,if=mtd,format=raw -nic user,tftp=/srv/tftp/
